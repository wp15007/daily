# 振り返り
---
## 1/26

### keep(今後も続けること)

- 参考になりそうなサイトの閲覧、ソース検証


### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- eclipseを使う

---
## 1/25

### keep(今後も続けること)

- 参考になりそうなサイトの閲覧、ソース検証


### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- eclipseを使う

---
## 1/22

### keep(今後も続けること)

- 参考になりそうなサイトの閲覧、ソース検証

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- 

---
## 1/21

### keep(今後も続けること)

- 参考になりそうなサイトの閲覧、ソース検証

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- 

---
## 1/20

### keep(今後も続けること)

- 参考になりそうなサイトの閲覧、ソース検証

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- 固定ページ追加

---
## 1/19

### keep(今後も続けること)

- 参考になりそうなサイトの閲覧、ソース検証

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- 固定ページの条件分岐の修正

---
## 1/18

### keep(今後も続けること)

- 参考になりそうなサイトの閲覧、ソース検証

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- レイアウト崩れの修正

---
## 1/15

### keep(今後も続けること)

- PHPの勉強
- 教科書以外の書籍を読む
- CSSの復習

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- グリッドレイアウト

---
## 1/14

### keep(今後も続けること)

- PHPの勉強
- 教科書以外の書籍を読む
- CSSの復習

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- ドロップダウンメニューの実装

---
## 1/13

### keep(今後も続けること)

- PHPの勉強
- 教科書以外の書籍を読む
- WordPressテーマ作成の情報収集

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- CSSの復習

---
## 1/12

### keep(今後も続けること)

- PHPの勉強
- 教科書以外の書籍を読む
- WordPressテーマ作成の情報収集

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- 

---
## 1/8

### keep(今後も続けること)

- PHPの勉強
- 教科書以外の書籍を読む
- WordPressテーマ作成の情報収集

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- 

---
## 1/7

### keep(今後も続けること)

- PHPの勉強
- 教科書以外の書籍を読む
- WordPressテーマ作成の情報収集

### problem(問題なのでやめること)

- 
### try(今後、試してみたいこと)

- オリジナルテーマの作り直し

---
## 1/6

### keep(今後も続けること)

- PHPの勉強
- 教科書以外の書籍を読む
- WordPressテーマ作成の情報収集

### problem(問題なのでやめること)

- [Bonesのカスタマイズ](http://bey.jp/?p=8745)

### try(今後、試してみたいこと)

- デフォルトテーマのカスタマイズ

---
## 1/5

### keep(今後も続けること)

- PHPの勉強
- 教科書以外の書籍を読む
- WordPressテーマ作成の情報収集

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- 公開テーマのカスタマイズ

---
## 1/4

### keep(今後も続けること)

- PHPの勉強
- 教科書以外の書籍を読む
- WordPressテーマ作成の情報収集

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- jQueryを使ったメニューの実装

---
## 12/28

### keep(今後も続けること)

- PHPの勉強
- 教科書以外の書籍を読む
- WordPressテーマ作成の情報収集

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

-

---
## 12/25

### keep(今後も続けること)

- PHPの勉強
- 教科書以外の書籍を読む
- WordPressテーマ作成の情報収集

### problem(問題なのでやめること)

- xampp再インストールの際にバックアップを取らなかったこと

### try(今後、試してみたいこと)

-

---
## 12/24

### keep(今後も続けること)

- PHPの勉強
- Javaの勉強
- 教科書以外の書籍を読む
- WordPressテーマ作成の情報収集

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

-

---
## 12/22

### keep(今後も続けること)

- PHPの勉強
- Javaの勉強
- 教科書以外の書籍を読む
- WordPressテーマ作成の情報収集

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- WordPressオリジナルテーマ作成

---

## 12/21

### keep(今後も続けること)

- PHPの勉強
- Javaの勉強
- 教科書以外の書籍を読む
- WordPressテーマ作成の情報収集

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- WordPressオリジナルテーマ作成

---
## 12/18

### keep(今後も続けること)

- PHPの勉強
- Javaの勉強
- 教科書以外の書籍を読む
- WordPressテーマ作成の情報収集

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- WordPressオリジナルテーマ作成

---
## 12/17

### keep(今後も続けること)

- PHPの勉強
- Javaの勉強
- 教科書以外の書籍を読む

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- WordPressテーマ作成の情報収集

---
## 12/16

### keep(今後も続けること)

- PHPの勉強
- Javaの勉強
- 教科書以外の書籍を読む

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- 

---
## 12/15

### keep(今後も続けること)

- PHPの勉強
- Javaの勉強
- 教科書以外の書籍を読む

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- 

---
## 12/14

### keep(今後も続けること)

- Javaの勉強
- 教科書以外の書籍を読む

### problem(問題なのでやめること)

- 途中であきらめたこと

### try(今後、試してみたいこと)

- 自宅で復習

---
## 12/11

### keep(今後も続けること)

- Javaの勉強
- 教科書以外の書籍を読む

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- 

---
## 12/10

### keep(今後も続けること)

- Javaの勉強
- 教科書以外の書籍を読む

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- 

---
## 12/9

### keep(今後も続けること)

- Javaの勉強
- 教科書以外の書籍を読む

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- 

---
## 12/8

### keep(今後も続けること)

- Javaの勉強
- 教科書以外の書籍を読む

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- MySQLを自宅で触ってみる

---
## 12/7

### keep(今後も続けること)

- Javaの勉強
- 教科書以外の書籍を読む

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- MySQLを自宅で触ってみる

---
## 12/4

### keep(今後も続けること)

- Javaの勉強
- 教科書以外の書籍を読む

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- 
---
## 12/3

### keep(今後も続けること)

- Webクリエイター試験対策
- Javaの勉強
- 教科書以外の書籍を読む

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- 

---
## 12/2

### keep(今後も続けること)

- Webクリエイター試験対策
- Javaの勉強
- 教科書以外の書籍を読む

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- 

---
## 12/1

### keep(今後も続けること)

- Webクリエイター試験対策
- Javaの勉強
- 教科書以外の書籍を読む

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- 

---
## 11/30

### keep(今後も続けること)

- Webクリエイター試験対策
- Javaの勉強
- 教科書以外の書籍を読む

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- 

---
## 11/27

### keep(今後も続けること)

- Webクリエイター試験対策
- Javaの勉強
- 教科書以外の書籍を読む

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- 

---
## 11/26

### keep(今後も続けること)

- Webクリエイター試験対策
- Javaの勉強
- 教科書以外の書籍を読む

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- 

---
## 11/25

### keep(今後も続けること)

- Webクリエイター試験対策
- Javaの勉強
- 教科書以外の書籍を読む

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- 

---

## 11/24

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- [ドットインストール](http://dotinstall.com/)
- 自宅での復習
- 教科書以外の書籍を読む
- Webクリエイター試験対策
- webサイト制作（休止中）

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- webサイト公開
- Javaの勉強

---
## 11/20

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- [ドットインストール](http://dotinstall.com/)
- 自宅での復習
- 教科書以外の書籍を読む
- Webクリエイター試験対策
- webサイト制作（休止中）

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- webサイト公開

---
## 11/18

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- [ドットインストール](http://dotinstall.com/)
- 自宅での復習
- 教科書以外の書籍を読む
- Webクリエイター試験対策
- webサイト制作（休止中）

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- webサイト公開


---
## 11/17

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- [ドットインストール](http://dotinstall.com/)
- 自宅での復習
- 教科書以外の書籍を読む
- Webクリエイター試験対策
- webサイト制作（休止中）

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- webサイト公開

---
## 11/16

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- [ドットインストール](http://dotinstall.com/)
- 自宅での復習
- 教科書以外の書籍を読む
- Webクリエイター試験対策
- webサイト制作（休止中）

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- webサイト公開

---
## 11/13

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- [ドットインストール](http://dotinstall.com/)
- 自宅での復習
- 教科書以外の書籍を読む
- Webクリエイター試験対策
- webサイト制作（休止中）

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- webサイト公開
- jQueryの予習

---
## 11/12

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- [ドットインストール](http://dotinstall.com/)
- 自宅での復習
- 教科書以外の書籍を読む
- Webクリエイター試験対策
- webサイト制作（休止中）

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- webサイト公開

---
## 11/11

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- [ドットインストール](http://dotinstall.com/)
- 自宅での復習
- 教科書以外の書籍を読む
- Webクリエイター試験対策
- webサイト制作（休止中）

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- webサイト公開

---
## 11/10

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- [ドットインストール](http://dotinstall.com/)
- 自宅での復習
- 教科書以外の書籍を読む
- Webクリエイター試験対策
- webサイト制作（休止中）

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- webサイト公開

---
## 11/9

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- [ドットインストール](http://dotinstall.com/)
- 自宅での復習
- 教科書以外の書籍を読む
- Webクリエイター試験対策
- webサイト制作（休止中）

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- webサイト公開


---

## 11/6

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- 自宅での復習
- 教科書以外の書籍を読む
- Webクリエイター試験対策
- webサイト制作（休止中）

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- [ドットインストール](http://dotinstall.com/)
- webサイト公開
- jobカード完成


---

## 11/5

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- 自宅での復習
- 教科書以外の書籍を読む
- Webクリエイター試験対策
- webサイト制作（休止中）

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- [ドットインストール](http://dotinstall.com/)
- webサイト公開


---

## 11/4

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- 自宅での復習
- 教科書以外の書籍を読む
- Webクリエイター試験対策
- webサイト制作（休止中）

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- [ドットインストール](http://dotinstall.com/)
- webサイト公開

---
## 11/1

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- 自宅での復習
- 教科書以外の書籍を読む
- Webクリエイター試験対策
- jQueryの勉強
- webサイト制作（休止中）

### problem(問題なのでやめること)

- 復習をしなかったこと

### try(今後、試してみたいこと)

- [ドットインストール](http://dotinstall.com/)
- webサイト公開

---
## 10/30

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- 自宅での復習
- 教科書以外の書籍を読む
- Webクリエイター試験対策
- jQueryの勉強
- webサイト制作（休止中）

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- [ドットインストール](http://dotinstall.com/)
- webサイト公開

---
## 10/29

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- 自宅での復習
- 教科書以外の書籍を読む
- Webクリエイター試験対策
- jQueryの勉強
- webサイト制作（休止中）

### problem(問題なのでやめること)

- 課題が未完成のままあきらめたこと

### try(今後、試してみたいこと)

- [ドットインストール](http://dotinstall.com/)
- webサイト公開

---
## 10/28

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- 自宅での復習
- 教科書以外の書籍を読む
- Webクリエイター試験対策
- jQueryの勉強
- webサイト制作（休止中）

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- webサイト公開
- [ドットインストール](http://dotinstall.com/)

---
## 10/27

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- 自宅での復習
- 教科書以外の書籍を読む
- Webクリエイター試験対策
- jQueryの勉強
- webサイト制作（休止中）

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- webサイト公開
- 

---
## 10/26

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- 自宅での復習
- 教科書以外の書籍を読む
- Webクリエイター試験対策
- jQueryの勉強
- webサイト制作（休止中）

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- webサイト公開

---
## 10/23

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- 自宅での復習
- 教科書以外の書籍を読む
- Webクリエイター試験対策
- jQueryの勉強
- webサイト制作（休止中）

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- webサイト公開

---
## 10/22

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- 自宅での復習
- 教科書以外の書籍を読む
- Webクリエイター試験対策
- jQueryの勉強
- webサイト制作（休止中）

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- webサイト公開

---
## 10/21

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- 自宅での復習
- webサイト制作
- 教科書以外の書籍を読む
- Webクリエイター試験対策

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- webサイト公開
- jQueryの勉強

---
## 10/20

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- 自宅での復習
- webサイト制作
- 教科書以外の書籍を読む
- Webクリエイター試験対策

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- webサイト公開

---
## 10/19

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- 自宅での復習
- webサイト制作
- 教科書以外の書籍を読む
- Webクリエイター試験対策

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- webサイト公開

---

## 10/16

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- 自宅での復習
- webサイト制作
- JavaScriptの勉強
- 教科書以外の書籍を読む


### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- webサイト公開
- webクリエイター試験のサンプル問題をやる

---
## 10/15

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- 自宅での復習
- webサイト制作
- JavaScriptの勉強
- 教科書以外の書籍を読む

### problem(問題なのでやめること)

- 昨日の復習をしなかった

### try(今後、試してみたいこと)

- webサイト公開
- webクリエイター試験のサンプル問題をやる

---

## 10/14

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- 自宅での復習
- webサイト制作
- JavaScriptの勉強
- 教科書以外の書籍を読む

### problem(問題なのでやめること)

-

### try(今後、試してみたいこと)

- webサイト公開
- webクリエイター試験のサンプル問題をやる

---
## 10/13

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- 自宅での復習
- webサイト制作
- JavaScriptの勉強
- 教科書以外の書籍を読む

### problem(問題なのでやめること)

-

### try(今後、試してみたいこと)

- webサイト公開
- webクリエイター試験のサンプル問題をやる

---
## 10/09

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- 自宅での復習
- webサイト制作
- JavaScriptの勉強
- 教科書以外の書籍を読む

### problem(問題なのでやめること)

- 前日の復習をしなかった

### try(今後、試してみたいこと)

- webサイト公開

---
## 10/08

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- 自宅での復習
- webサイト制作
- JavaScriptの勉強
- 教科書以外の書籍を読む

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- webサイト公開

---
## 10/07

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- 自宅での復習
- webサイト制作
- JavaScriptの勉強
- 教科書以外の書籍を読む

### problem(問題なのでやめること)

- 昼食の食べ過ぎ（午後眠くなる）

### try(今後、試してみたいこと)

- webサイト公開

---
## 10/06

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)
- [progate](http://prog-8.com/)
- 自宅での復習
- webサイト制作
- JavaScriptの勉強
- 教科書以外の書籍を読んでみる


### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- webサイト公開

---
## 09/30

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)のスキマ学習
- 自宅での復習
- webサイト制作
- LAMP環境を理解する
- JavaScriptの勉強
- 教科書以外の書籍を読んでみる
- [GoogleAnalytics](https://www.google.com/intl/ja_jp/analytics/)の機能を試す

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- webサイト公開
- [progate](http://prog-8.com/)

---
## 09/29

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)のスキマ学習
- 自宅での復習
- webサイト制作
- エラーのチェック（エラー箇所の前後のプログラムまでチェックする）
- LAMP環境を理解する
- JavaScriptの勉強
- 教科書以外の書籍を読んでみる
- [GoogleAnalytics](https://www.google.com/intl/ja_jp/analytics/)の機能を試す

### problem(問題なのでやめること)

- keepが増えすぎたので減らす(e-typing)

### try(今後、試してみたいこと)

- webサイト公開

---
## 09/28

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)のスキマ学習
- [e-typing](http://www.e-typing.ne.jp/english/)のスキマ学習
- 自宅での復習
- webサイト制作
- エラーのチェック（エラー箇所の前後のプログラムまでチェックする）
- LAMP環境を理解する
- JavaScriptの勉強
- 教科書以外の書籍を読んでみる

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- [GoogleAnalytics](https://www.google.com/intl/ja_jp/analytics/)を使いこなす

---
## 09/25

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)のスキマ学習
- [e-typing](http://www.e-typing.ne.jp/english/)のスキマ学習
- 自宅での復習
- webサイト制作
- エラーのチェック（エラー箇所の前後のプログラムまでチェックする）
- LAMP環境を理解する
- JavaScriptの勉強

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- 教科書以外の書籍を読んでみる
---
## 09/24

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)のスキマ学習
- [e-typing](http://www.e-typing.ne.jp/english/)のスキマ学習
- 自宅での復習
- webサイト制作
- エラーのチェック（エラー箇所の前後のプログラムまでチェックする）
- LAMP環境を理解する

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- Apache2,MySQL,PHP,WordPressのインストール
- JavaScriptの勉強

---
## 09/18

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)のスキマ学習
- [e-typing](http://www.e-typing.ne.jp/english/)のスキマ学習
- 自宅での復習
- webサイト制作
- エラーのチェック（エラー箇所の前後のプログラムまでチェックする）

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- 「LAMP環境」を理解する

---
## 09/17

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)のスキマ学習
- [e-typing](http://www.e-typing.ne.jp/english/)のスキマ学習
- 自宅での復習
- webサイト制作
- エラーのチェック（エラー箇所の前後のプログラムまでチェックする）

### problem(問題なのでやめること)

- タイプミス（←タイプミスは避けられないのでエラーのチェックをしっかりする）

### try(今後、試してみたいこと)

- 

---
## 09/16

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)のスキマ学習
- [e-typing](http://www.e-typing.ne.jp/english/)のスキマ学習
- 自宅での復習
- webサイト制作

### problem(問題なのでやめること)

- タイプミス

### try(今後、試してみたいこと)

- 
---
## 09/15

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)のスキマ学習
- [e-typing](http://www.e-typing.ne.jp/english/)のスキマ学習
- 自宅での復習
- webサイト制作

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- Genymotionでgoogleplayを使ってみる
---
## 09/14

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)のスキマ学習
- [e-typing](http://www.e-typing.ne.jp/english/)のスキマ学習
- 自宅での復習
- webサイト制作

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- 自宅PCでGenymotionのインストール
---
## 09/11

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)のスキマ学習
- [e-typing](http://www.e-typing.ne.jp/english/)のスキマ学習
- 自宅での復習
- webサイト制作

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- 
---
## 09/10

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)のスキマ学習
- [e-typing](http://www.e-typing.ne.jp/english/)のスキマ学習
- 自宅での復習
- webサイト制作

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- 

---
## 09/09

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)のスキマ学習
- [e-typing](http://www.e-typing.ne.jp/english/)のスキマ学習
- 自宅での復習

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- とりあえずwebサイトを作ってみる

---
## 09/08

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)のスキマ学習
- [e-typing](http://www.e-typing.ne.jp/english/)のスキマ学習

### problem(問題なのでやめること)

- 資格試験までの道のりが見えていない　　

### try(今後、試してみたいこと)

- [CODEPREP](https://codeprep.jp/)のHTMLとCSSはヒント無しでやってみる
- 資格試験までのスケジュールを立ててみる

---

## 09/07

### keep(今後も続けること)

- [CODEPREP](https://codeprep.jp/)を毎日コツコツやる

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- 空き時間に[e-typing](http://www.e-typing.ne.jp/english/)をやってみる

---

## 09/04

### keep(今後も続けること)

- 毎日振り返りを書く

### problem(問題なのでやめること)

- 

### try(今後、試してみたいこと)

- emmetを使いこなす

---